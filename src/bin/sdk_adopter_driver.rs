...
fn checkout(lock: &BackendLock, cache: &Path, offline: bool) -> Result<PathBuf> {
    ...
    if !directory.join(".git").is_dir() {
        if offline {
            return Err(err(
                ...
                format!(
                    "missing pinned checkout {}; bootstrap once without --offline",
                    directory.display()
                ),
            ));
        }
        ...
        run(
            "backend.checkout",
            Command::new("git").args(["-C"]).arg(&directory).args([
                "remote",
                "add",
                "origin",
    ...
    if String::from_utf8_lossy(&head.stdout).trim() != lock.revision {
        return Err(err(
            "backend.pin",
            format!(
                "cached source {} is not the locked revision {}; do not use a mutable checkout",
                directory.display(),
                lock.revision
            ),
        ));
    ...
    if !status.stdout.is_empty() {
        return Err(err(
            "backend.pin",
            format!(
                "cached backend source has uncommitted modifications: {}",
                directory.display()
            ),
        ));
    }
    ...
    if !executable.is_file() {
        if offline {
            return Err(err(
                "backend.offline_cache",
                format!(
                    "missing compiled backend {}; bootstrap once without --offline",
                    executable.display()
                ),
            ));
        }
...
impl BackendDriver for UpstreamDriver {
    fn generate(
        ...
        write_if_changed(
            &work.join("openapi-to-rust.toml"),
            config.as_bytes(),
            "raw.config",
        )?;
        run(
            "raw.generate",
            Command::new(binary(lock, cache, offline)?)
        ...
        for name in &lock.generated_layout {
            if !raw.join(name).is_file() {
                return Err(err(
                    "raw.layout",
                    format!(
                        "pinned backend did not produce {}",
                        raw.join(name).display()
                    ),
                ));
            }
        }
        ...
        for entry in fs::read_dir(&raw).map_err(|e| err("raw.layout", e))? {
            ...
            if path.extension().is_some_and(|value| value == "rs") {
                let name = entry
                    .file_name()
                    .into_string()
                    .map_err(|_| err("raw.layout", "non-UTF-8 generated source filename"))?;
                if !["client.rs", "types.rs", "mod.rs"].contains(&name.as_str()) {
                    return Err(err(
                        "raw.layout",
                        format!("unreviewed raw Rust layout: {name}"),
                    ));
                }
        ...
        let dependencies = fs::read_to_string(raw.join("REQUIRED_DEPS.toml"))
            .map_err(|e| err("raw.dependencies", e))?;
        if !dependencies
            .lines()
            .any(|line| line.trim() == "[dependencies]")
        {
            return Err(err(
                "raw.dependencies",
