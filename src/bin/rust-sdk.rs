...
fn stage_path(dest: &Path) -> Result<Stage> {
    let stamp = SystemTime::now()
        ...
        .as_nanos();
    let stage = dest.with_file_name(format!(".rust-sdk-stage-{}-{stamp}", std::process::id()));
...
fn args() -> Result<Cli> {
    ...
    let command = iter.next().ok_or_else(|| err("cli.usage", "rust-sdk init --openapi FILE --output DIR --name CRATE [--surface FILE] [--overrides FILE] [--offline] | sync --crate DIR [--check] [--accept-coverage] [--offline]"))?;
...
fn main_inner() -> Result<()> {
    ...
    // An unchanged effective source and locked recipe must never rewrite
    // different generated bytes. Even a still-marked facade may have been
