...
JSON errors, and buffered/streaming binary export. It uses no downstream
...
raw-backend revision in `openapi-to-rust-bindings/COMPATIBILITY.json`, builds
...
and streaming binary responses. It imports neither the generator, adapter nor
...
| --- | --- | --- |
| `[backend checkout]`, `[backend pin]`, `[backend build]`, `[raw backend]` | Pinned raw backend and its generated manifest/Rust | `_backend/`, `first/raw/`, `first/openapi-to-rust.toml` |
| `[bindings adapter]` | Manifest to canonical Bindings v3 | `first/raw/binding-manifest.json`, `first/rust-bindings.json` |
