...
| `.github/workflows/openapi-to-rust-compat.yml` | Schedules daily, dispatches candidate ref, builds two fork revisions and invokes manifest-only compatibility checker | This preparatory branch isolates **legacy oracle** fixtures and exposes stage reports. Production retarget to upstream revisions and the new default is blocked on #155/#156. |
...
The candidate revision itself can be reproduced by manual workflow dispatch with `candidate_ref=9fee7af67c05d896da86e41d326b067312120c53`. A legacy incompatible result may be intentional; inspect the report rather than updating the backend pin automatically.
...
2. After #156: retarget the ordinary/nightly/manual tracker and `COMPATIBILITY.json` to the same upstream repository, pinned baseline, exact effective OpenAPI and `--extract` contract as the default SDK CLI. Implement a separate production compatibility runner rather than silently letting the manifest-only oracle read generic fixtures; remove moving fork manifest dependency from scheduled checks.
