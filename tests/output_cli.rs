...
fn freshness_check_is_read_only_and_reports_changed_missing_and_extra_files() {
    ...
    let first_diff: serde_json::Value = serde_json::from_slice(&first.stdout).expect("JSON diff");
    assert!(
        first_diff["missing"]
            .as_array()
            .expect("missing list")
    ...
    assert_eq!(
        fs::read(output.join("mod.rs")).unwrap(),
        changed,
...
fn freshness_check_reports_handwritten_path_conflict_without_overwriting() {
    ...
    let diagnostic: serde_json::Value =
        serde_json::from_slice(&generate.stderr).expect("JSON error");
