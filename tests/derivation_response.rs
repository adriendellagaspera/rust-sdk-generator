...
fn derives_owned_scalar_response_view_and_generates_it() {
    let openapi: OpenApi = serde_json::from_str(include_str!(
        "fixtures/derivation-response/openapi.json"
    ))
    ...
    let surface: PublicSdkSurface = serde_json::from_str(include_str!(
        "fixtures/derivation-response/surface.json"
    ))
    ...
    let accessors = response.accessors.as_ref().expect("response view accessors");
    assert_eq!(
    ...
    assert_eq!(operation.response.as_deref(), Some("GetFleetSensorsResponse"));
    ...
    assert_eq!(generated.inventory.resources[1].path, vec!["fleet", "sensors"]);
